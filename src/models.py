...
class nucSimpleCnnOrig(nn.Module):
    def __init__(self, input_size, hidden_size, dropout=0.005, num_classes=2, **kwargs):
        ...
        self.conv1d1 = nn.Conv1d(4, hidden_size, 11, stride=1, padding=5, bias=True)
        
        self.conv1d2 = nn.Conv1d(hidden_size, hidden_size, 7, stride=1, padding=3, groups=1, bias=True)
        
        self.pool2 = nn.MaxPool1d(kernel_size=2)

        self.conv1d3 = nn.Conv1d(hidden_size, hidden_size * 8, 7, stride=1, padding=3, groups=1, bias=True)
...
class nucSimpleCnn(nn.Module):
    def __init__(self, input_size = 200, hidden_size = 64, dropout = 0.5, num_classes = 2, **kwargs):
        ...
        self.conv1d1 = nn.Conv1d(1, hidden_size, 11, stride=5, padding=1, bias=True)
        
        self.conv1d2 = nn.Conv1d(hidden_size, hidden_size, 1, stride=1, padding=1, groups=1, bias=True)
...
class SimpleCnn(nn.Module):
    def __init__(self, input_size = 200, hidden_size = 64, dropout = 0.5, num_classes = 2, **kwargs):
        ...
        self.conv1d1 = nn.Conv1d(in_channels=3, out_channels=hidden_size, kernel_size=1, padding=0)
        
        self.conv1d2 = nn.Conv1d(hidden_size, hidden_size, kernel_size=3, padding=1)
        
        self.pool2 = nn.MaxPool1d(kernel_size=2)

        self.conv1d3 = nn.Conv1d(hidden_size, int(hidden_size * 2), kernel_size=3, padding=1)
...
class CnnLinear(nn.Module):
    def __init__(self, input_size, hidden_size, linear_size, dropout=0.5, num_classes=2, **kwargs):
        ...
        self.fc_meta1 = nn.Linear(in_features=linear_size, out_features=int(hidden_size / 4), bias=False)

        self.fc_meta2 = nn.Linear(in_features=int(hidden_size / 4), out_features=int(hidden_size / 4))

        self.fc_meta3 = nn.Linear(in_features=int(hidden_size / 4), out_features=int(hidden_size / 2))

        self.fc_meta4 = nn.Linear(in_features=int(hidden_size / 2), out_features=int(hidden_size / 2))

        self.fc_meta5 = nn.Linear(in_features=int(hidden_size / 2), out_features=hidden_size)

        self.fc_combine = nn.Linear(in_features=int(hidden_size * 2), out_features=int(hidden_size / 2))

        self.fc_combine2 = nn.Linear(in_features=int((hidden_size / 2)), out_features=int(num_classes))
...
class LSTM_Classifier(nn.Module):
    def __init__(self, num_classes = 2, input_size = 200, hidden_size = 64, num_layers = 10, seq_length = 200, dropout = 0.5, batch_size = 512):
        ...
        self.lstm = nn.LSTM(input_size = self.seq_length, hidden_size = hidden_size, num_layers=self.num_layers, batch_first=True) #lstm
        self.fc_1 = nn.Linear(self.hidden_size, int(self.hidden_size / 2)) #fully connected 1
        self.fc = nn.Linear(int(self.hidden_size / 2), num_classes) #fully connected last layer
...
class ConvLSTM_Classifier(nn.Module):
    def __init__(self, num_classes = 2, input_size = 200, hidden_size = 64, num_layers = 10, seq_length = 200, dropout = 0.5, batch_size = 512):
        ...
        self.lstm = nn.LSTM(input_size = self.seq_length, hidden_size = hidden_size, num_layers=self.num_layers, batch_first=True) #lstm
        self.conv1d1 = nn.Conv1d(in_channels = 3, out_channels = 1, kernel_size=1, padding=0)
        self.fc_1 = nn.Linear(self.hidden_size, int(self.hidden_size / 2)) #fully connected 1
        self.fc = nn.Linear(int(self.hidden_size / 2), num_classes) #fully connected last layer
...
class AttentionNetwork(nn.Module):
    def __init__(self, input_size = 200, hidden_size = 64, k_max = 20, dropout = 0.5, num_classes = 2, hidden_attention = 128):
        ...
        self.transformer_part2 = nn.Sequential(
            nn.Linear(self.k_max * self.hidden_CNN, self.input_size), #match the first numer with the output of cnn after pooling 20*
            nn.ReLU(),
            nn.Dropout(p = self.dropout)
        )
