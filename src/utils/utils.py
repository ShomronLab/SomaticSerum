...
class Parms:
    ...
    def return_model(self, meta_dim):
        Models = {
            ...
            'CnnLinear':            Model.CnnLinear(input_size=self.INPUT_SIZE, hidden_size=self.HIDDEN, linear_size=meta_dim, dropout=self.DROPOUT, num_classes=self.NUM_CLASSES),
            ...
            'metaSimpleDense':      Model.metaSimpleDense(input_size=meta_dim)
        }
        return Models[self.MODEL]

    def return_output_dir(self, args):
        out = os.path.join('..', 'results', "{}/{}_epoch{}dropout{}hidden{}batchsize{}_{}".format(args.out, self.MODEL,
...
def split_data(data_path, train_size=0.8, val_size=0.1, test_size=0.1):
    ...
    bam_numbers = list(
        set([int(re.search(r'\d+', s).group()) for s in onlybams]))
    ...
    logging.info("train_samples: {}, count {}".format(
        train_samples, len(train_samples)))
    logging.info("val_samples: {}, count {}".format(
        val_samples, len(val_samples)))
    logging.info("test_samples: {}, count {}".format(
...
def plot(input_file, metric, metric_test, out):
    ...
    with plt.style.context('bmh'):
        ...
        df['train_acc'].plot(color='b', linestyle='solid',
                             ax=ax2, label='train_acc')
        df['val_acc'].plot(color='b', linestyle='-.', ax=ax2, label='val_acc')
        df['train_loss'].plot(color='g', linestyle='solid',
                              ax=ax1, label='train_loss')
        df['val_loss'].plot(color='g', linestyle='-.',
                            ax=ax1, label='val_loss')
        ...
        plt.subplots_adjust(top=0.8, bottom=0.1, left=0.1,
                            right=0.9, hspace=0.5, wspace=0.5)
