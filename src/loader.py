...
class CustomBamDataset2(torch.utils.data.Dataset):
    ...
    cigar_dict = {  "M": 0.1,  # M 0 alignment match (can be a sequence match or mismatch)
                    ...
                    "S": 0.5,  # S 4 soft clipping (clipped sequences present in SEQ)
                    "H": 0.6,  # H 5 hard clipping (clipped sequences NOT present in SEQ)
                    "P": 0.7,  # P 6 padding (silent deletion from padded reference)
                    "=": 0.8,  # = 7 sequence match
                    "X": 0.9   # X 8 sequence mismatch
                }

    def __init__(self, bam_file_location, out = None, whichSet = "test_loader", read_length = 200, input_files_path = None):
        ...
        if os.path.exists("{}/data_{}.pkl".format(self.pickles, self.whichSet)):
            with open("{}/data_{}.pkl".format(self.pickles, self.whichSet), 'rb') as f:
                self.number_of_normal_reads, self.number_of_somatic_reads, self.seqs = pickle.load(f)
        else:
            ...
            self.seqs = []
            self.genSet()
            with open("{}/data_{}.pkl".format(self.pickles, self.whichSet), 'wb') as f:
                pickle.dump((self.number_of_normal_reads, self.number_of_somatic_reads, self.seqs), f)
    ...
    def longest(self):
        ...
        for bam_number, bam_file in enumerate(self.bam_files):
            bam = pysam.AlignmentFile(os.path.join(self.bam_dir, bam_file), 'rb')
            for read in bam.fetch():
                if read.template_length > 1000: continue
    ...
    def genSet(self):
        ...
        for bam_number, bam_file in enumerate(self.bam_files):
            bam = pysam.AlignmentFile(os.path.join(self.bam_dir, bam_file), 'rb')
            logging.info("Loading reads from: {} \t\tbam file number {}".format(bam_file, bam_number))
            for read in bam.fetch():
                ...
                def genertor(read):
                    ...
                    for cigar, seq_len in zip(cigars, seqs_lengths):
                        if cigar in ['H', 'D', 'P']:
                            ...
                            quals = [round((ord(x)-33)/93, 3) for x in read_dict['qual'][k:seq_len+k]]
                    ...
                    tlen = 0 if abs(read.template_length) > 1000 else abs(read.template_length)
                    meta = [round(tlen/self.longest_read, 3), 
                            int(read.is_reverse), 
                            int(read.is_read1), 
                            int(read.is_read2), 
                    ...
                    # logging.info("seqs_lengths", seqs_lengths, sum(seqs_lengths))
...
if "__main__" == __name__:
    test = CustomBamDataset2("/data/hadasvol/projects/cancer_plasma/seqmerge/DLbams")