...
def train(model, optimizer, loss_fn, train_dl, val_dl, epochs=100, device='cpu', out='.'):
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch in train_dl:
            ...
            CM_train += confusion_matrix(labels.cpu(),
                                         preds.cpu(), labels=[0, 1])

        train_acc = num_train_correct / num_train_examples
        train_loss = train_loss / len(train_dl.dataset)
        metric.loc[len(metric)] = list(
            util.get_classification_success_stats(epoch, CM_train, 'train'))
        ...
        for batch in val_dl:
            ...
            outputs = yhat
            preds = torch.argmax(outputs.data, 1)
            CM_val += confusion_matrix(labels.cpu(),
                                       preds.cpu(), labels=[0, 1])

        val_acc = num_val_correct / num_val_examples
        val_loss = val_loss / len(val_dl.dataset)
        cur_val_metric = list(
            util.get_classification_success_stats(epoch, CM_val, 'val'))
        ...
        if epoch > epochs/2 and val_acc > best_val_acc:
            ...
            try:
                ...
                logging.info(
                    'Saved model with best validation accuracy: %5.4f' % best_val_acc)
            except:
                logging.info(
                    'Failed to save model with best validation accuracy: %5.4f' % best_val_acc)
    ...
    f.write('\n%d,%5.2f,%5.2f,%5.2f,%5.2f' %
            (epoch, train_loss, train_acc, best_val_loss, best_val_acc))
    history['loss'].append(train_loss)
