...
class crnn_Classifier(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, target_size=2, bidirectional=False, n_layer=2):
        super(crnn_Classifier, self).__init__()
        self.hidden_dim = hidden_dim
        self.n_layers = 4 # 2, 4, 6 ,8
        if self.n_layers >= 2:
            # self.conv1 = nn.Conv1d(in_channels=embedding_dim, out_channels=hidden_dim, kernel_size=3, padding=)
            self.conv1 = nn.Conv1d(4, hidden_dim, 11, stride=5, padding=1)
            self.conv2 = nn.Conv1d(hidden_dim, hidden_dim, 1, stride=1, padding=1)
            self.bn1 = nn.BatchNorm1d(num_features=self.hidden_dim)
            self.bn2 = nn.BatchNorm1d(num_features=self.hidden_dim)
            self.p1 = nn.MaxPool1d(kernel_size=2)
            self.scale_pool = 2
        if self.n_layers >= 4:
            self.conv3 = nn.Conv1d(hidden_dim, int(hidden_dim*8), 7, stride=1, padding=3)
            # self.conv3 = nn.Conv1d(hidden_dim, hidden_dim, 3, padding=1)
            self.conv4 = nn.Conv1d(in_channels=int(hidden_dim*8), out_channels=hidden_dim, kernel_size=7, padding=1)
            self.bn3 = nn.BatchNorm1d(num_features=int(hidden_dim*8))
            self.bn4 = nn.BatchNorm1d(num_features=self.hidden_dim)
            self.p2 = nn.MaxPool1d(kernel_size=2)
            self.scale_pool = 4
        if self.n_layers >= 6:
        ...
        self.grad_clipping = 10.
        # The linear layer that maps from hidden state space to tag space
        self.lc1 = nn.Linear(int(hidden_dim/8), int(hidden_dim*64))
        self.lc2 = nn.Linear(int(hidden_dim*64), int(hidden_dim*32))
        self.lc3 = nn.Linear(int(hidden_dim*32), int(hidden_dim*8))
        self.lc4 = nn.Linear(int(hidden_dim*8), target_size)

    def forward(self, landmarks, lengths=64):
        landmarks = landmarks.permute(0, 1, 2)  # (b, seq, dim) --> (b, dim, seq)
        # Convolve on Seq for each dim to get (b, dim, seq)
        if self.n_layers == 8:
            landmarks = F.relu(self.bn8(self.conv8(F.relu(self.bn7(self.conv7(self.p3(F.relu(self.bn6(self.conv6(F.relu(self.bn5(self.conv5(self.p2(F.relu(self.bn4(self.conv4(F.relu(self.bn3(self.conv3(self.p1(F.relu(self.bn2(self.conv2(F.relu(self.bn1(self.conv1(landmarks)))))))))))))))))))))))))))
        elif self.n_layers == 6:
            ...
            # landmarks = self.p2(F.relu(self.bn4(self.conv4(F.relu(self.bn3(self.conv3(self.p1(F.relu(self.bn2(self.conv2(F.relu(self.bn1(self.conv1(landmarks))))))))))))))
            landmarks = self.conv1(landmarks)
            landmarks = self.bn1(landmarks)
            landmarks = F.relu(landmarks)
            landmarks = self.conv2(landmarks)
            landmarks = self.bn2(landmarks)
            landmarks = F.relu(landmarks)
            landmarks = self.p1(landmarks)
            landmarks = self.conv3(landmarks)
            landmarks = self.bn3(landmarks)
            landmarks = F.relu(landmarks)
            landmarks = self.conv4(landmarks)
            landmarks = self.bn4(landmarks)
            landmarks = F.relu(landmarks)
            landmarks = self.p2(landmarks)
        ...
        # landmarks = landmarks.permute(0, 1, 2)
        # Feed into GRU
        # import pdb; pdb.set_trace()
        ...
        #     ht.register_hook(lambda x: x.clamp(min=-self.grad_clipping, max=self.grad_clipping))
        # ht = self.dropout(ht[-1])
        logit = F.relu(self.lc1(landmarks))
        logit = self.lc2(logit)
        logit = self.lc3(logit)
        logit = self.lc4(logit)
        logit = torch.flatten(logit, 1)
