...
import torch
# from loader import CustomBamDataset2
from torch._C import device
from torch.utils.data import DataLoader

from bam_dataloader import CustomBamDataset
...
def split_data(data_path, train_size=0.8, val_size=0.1, test_size=0.1):
    ...
    inds = set(random.sample(list(range(len(bam_numbers))),
               int((1-train_size)*len(bam_numbers))))
...
def main(args):
    ...
    optimizer = torch.optim.AdamW(
        params, lr=learning_rate, eps=1e-08, weight_decay=0.01)
...
def test_bam_loading(training_bam_dir):
    example_dataset = CustomBamDataset(
        training_bam_dir, args.out, N_CHANNELS, bam_lines_maximum=100)
    train_size = int(0.8 * len(example_dataset))
    test_size = len(example_dataset) - train_size
    train_dataset, test_dataset = torch.utils.data.random_split(
        example_dataset, [train_size, test_size])
